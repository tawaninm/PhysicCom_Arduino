...
const char PUBLISH_TOPIC[] = "67070069/temp";       // CHANGE IT AS YOU DESIRE
const char SUBSCRIBE_TOPIC[] = "67070069/temp";  // CHANGE IT AS YOU DESIRE
...
void sendToMQTT() {
  // อ่านค่าเซนเซอร์จาก A0
  int sensorValue = analogRead(A0);              // 0..4095
  float voltage = sensorValue * (5.0 / 1023.0);   // ใช้ถ้า analogRead ได้ 0..1023
  float temperatureC = (voltage - 0.5) * 100.0;

  // เตรียมข้อความเพื่อส่งขึ้น MQTT
  // ทำเป็นข้อความเดียว เช่น "temp:25.43C"
  char messageBuffer[32];
  snprintf(messageBuffer, sizeof(messageBuffer), "%.2f", temperatureC);
