...
void sendToMQTT() {
  ...
  // อ่านค่าจาก pot (0-1023)
  int potValue = analogRead(potPin);   // ถ้า UNO R4 ของคุณอ่านได้ 0-4095 แล้วอยากได้ 0-1023 จริงๆ ให้ map ด้านล่าง

  // ถ้าอยากบังคับให้เป็น 0-1023 เสมอ แม้ ADC จะเป็น 12-bit
  // int potValueRaw = analogRead(potPin);       // 0..4095
  // int potValue = map(potValueRaw, 0, 4095, 0, 1023);

  // แปลงเป็นสตริงเพื่อส่ง MQTT
  char messageBuffer[16];
  snprintf(messageBuffer, sizeof(messageBuffer), "%d", potValue);

  // ส่งขึ้น MQTT
  mqtt.publish(PUBLISH_TOPIC, messageBuffer);

  // debug ใน Serial
  Serial.println("Arduino UNO R4 - sent to MQTT (POT):");
  Serial.print("- topic: ");
